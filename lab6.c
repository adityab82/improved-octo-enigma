...
void main(void)
{
	...
	while(1)
	{
		for(i = 0; i < 1000; i++)
		{
        	...
        	voltage=adcval*3.187/1023.0;
        	y = 100.0*(voltage - 2.73);			//y is now the temperature
		...
		}							// the code exits the for loop after 10 seconds of displaying the temperature
		
		float mean = sum/10000;
		printf("%f\n", mean);
		waitms(2000);				//displays mean on putty for 2 seconds and then goes back to mesauring temperature
	}
}
